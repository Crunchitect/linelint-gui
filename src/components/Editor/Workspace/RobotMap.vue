...
import { heuristic, weight } from '@/config/pathfind';
import type { AStarPath, BinaryImage } from '@/lib/types';
import { detect_junctions } from '@/lib/junctiondetect';
...
const processImage = async () => {
    ...
    draw();
    detect_junctions(thinned_image, <AStarPath>path);
};
...
<template>
    <div class="flex justify-center items-center bg-base w-full h-[95%]">
        <input class="w-4/5 h-4/5 opacity-0 absolute" accept="image/png, image/jpeg" type="file" @change="updateFile"
            @click.prevent>
        <canvas ref="canvas" class="rounded border-outline border-2 max-w-[50vw] max-h-[50vh]"
            v-show="hasPicture"></canvas>
